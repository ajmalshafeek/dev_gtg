...
class Cronjob_model extends CI_Model
{
    ...
    public function due_mail()
    {
        ...
        $this->db->where('DATE(gtg_invoices.invoiceduedate)<=', $duedate);
        $this->db->where('gtg_invoices.status', 'due');
    ...
    public function reports()
    {
        ...
        $query = $this->db->query("SELECT MONTH(invoicedate) AS month,YEAR(invoicedate) AS year,COUNT(tid) AS invoices,SUM(total) AS sales,SUM(items) AS items FROM gtg_invoices WHERE (YEAR(invoicedate)='$year') GROUP BY MONTH(invoicedate)");
        $arrayA = $query->result_array();

        $query = $this->db->query("SELECT MONTH(date) AS month,YEAR(date) AS year,SUM(credit) AS income,SUM(debit) AS expense FROM gtg_transactions WHERE (YEAR(date)='$year') GROUP BY MONTH(date)");
        ...
        $this->db->insert_batch('gtg_reports', $batch);
    ...
    public function subs()
    {
        ...
        if ($config == 0) {
            ...
            foreach ($result as $row) {
                ...
                $this->db->where('gtg_invoice_items.tid', $last_id);
                $query = $this->db->get();
