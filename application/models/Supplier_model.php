...
class Supplier_model extends CI_Model
{
    ...
    public function add($name, $company, $phone, $email, $address, $city, $region, $country, $postbox, $taxid)
    {
        ...
        if ($this->db->insert('gtg_supplier', $data)) {
    ...
    {
        $query = $this->db->query("SELECT c.*,p.pc FROM gtg_cust_group AS c LEFT JOIN ( SELECT gid,COUNT(gid) AS pc FROM gtg_supplier GROUP BY gid) AS p ON p.gid=c.id");
    ...
    {
        $this->db->select('gtg_purchase.*');
        $this->db->from('gtg_purchase');
        $this->db->where('gtg_purchase.csd', $id);
