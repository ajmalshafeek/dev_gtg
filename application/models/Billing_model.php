...
class Billing_model extends CI_Model
{
    ...
    public function paynow($tid, $amount, $note, $pmethod, $loc = false, $bill_date = null, $account_d = 0)
    {
        ...
        if (!$account['id']) {
            ...
            $this->db->join('gtg_accounts', 'univarsal_api.key1 = gtg_accounts.id', 'left');
        ...
        $this->db->select('gtg_invoices.*,gtg_customers.name,gtg_customers.id AS cid');
        $this->db->from('gtg_invoices');
        $this->db->where('gtg_invoices.id', $tid);
    ...
    public function gateway_update($gid, $currency, $key1, $key2, $enable, $devmode, $p_fee)
    {
        ...
        if ($this->db->update('gtg_gateways')) {
    ...
    public function bank_ac_add($name, $acn, $code, $enable, $bank, $branch, $address)
    {
        ...
        if ($this->db->insert('gtg_bank_ac', $data)) {
            echo json_encode(array('status' => 'Success', 'message' =>
    ...
    public function add_currency($code, $symbol, $spos, $rate, $decimal, $thous_sep, $deci_sep)
    {
        ...
        if ($this->db->insert('gtg_currencies', $data)) {
            echo json_encode(array('status' => 'Success', 'message' =>
            $this->lang->line('ADDED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' =>
    ...
    public function edit_currency($gid, $code, $symbol, $spos, $rate, $decimal, $thous_sep, $deci_sep)
    {
        ...
        $this->db->set($data);
        $this->db->where('id', $gid);
        if ($this->db->update('gtg_currencies')) {
            echo json_encode(array('status' => 'Success', 'message' =>
            $this->lang->line('UPDATED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' =>
    ...
    public function token($in = 0, $type = 1)
    {
        if ($type == 1) {
            ...
            $this->db->delete('gtg_metadata', array('type' => 71, 'rid' => $in));
        }
    }
}
