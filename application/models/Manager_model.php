...
class Manager_model extends CI_Model
{
    ...
    private function _project_datatables_query($cday = '')
    {
        ...
        $this->db->where('gtg_project_meta.meta_data', $this->aauth->get_user()->id);
        if ($cday) {
    ...
    public function project_stats($project)
    {
        ...
        $query = $this->db->query("SELECT
				...
				COUNT(IF( gtg_projects.status = 'Finished', gtg_projects.id, NULL)) AS Finished
				FROM gtg_projects LEFT JOIN gtg_project_meta ON gtg_project_meta.pid=gtg_projects.id WHERE gtg_project_meta.meta_key=19 AND gtg_project_meta.meta_data=" . $this->aauth->get_user()->id . "");
    ...
    public function task_stats($id)
    {
        $query = $this->db->query("SELECT
				...
				FROM gtg_todolist WHERE related=1 AND rid=$id AND aid=" . $this->aauth->get_user()->id . "");
    ...
    public function explore($id)
    {
        ...
        $this->db->join('gtg_invoices', 'gtg_project_meta.meta_data = gtg_invoices.tid', 'left');
        $query = $this->db->get();
